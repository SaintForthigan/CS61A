...
;; removed double load check

;; Make number->string remove leading "+" if necessary

;; removed + hack for Racket
  ...
  (let ([string? string?]
    ...
    (define (error-printform x)
      (if (string? x)
          (string-append "\"" x "\"")
          x))
    (lambda (string . args) (apply error (cons string (map error-printform args))))))
...
(define word?
  (let ([number? number?]
        [symbol? symbol?]
        [string? string?])
    (lambda (x) (or (symbol? x) (number? x) (string? x)))))
...
(define empty?
  (let ([null? null?]
        [string? string?]
        [string=? string=?])
    (lambda (x) (or (null? x) (and (string? x) (string=? x ""))))))
    ...
    (define (rank-string str rank)
      (define (helper i len)
        (if (= i len)
            'done
            (begin
              (vector-set! *the-char-ranks* (char->integer (string-ref str i)) rank)
              (helper (+ i 1) len))))
  ...
  (let ([= =]
    ...
    (lambda (string)
      (define (subsequents? string i length)
        (cond
          [(= i length) #t]
          [(<= (char-rank (string-ref string i)) 2) (subsequents? string (+ i 1) length)]
          [else #f]))
      ...
      (define (ok-symbol? string)
        (if (string=? string "")
            ...
            (let ([rank1 (char-rank (string-ref string 0))])
              (cond
                [(= rank1 0) (subsequents? string 1 (string-length string))]
      ...
      (define (nn-helper string i len seen-point?)
        (cond
          ...
          [(= 2 (char-rank (string-ref string i))) (nn-helper string (+ i 1) len seen-point?)]
          [else #f]))
        ...
        (if (string=? string "")
            ...
            (let* ([c0 (string-ref string 0)]
              ...
              (if (and (char=? c0 #\-) (not (= len 1)))
                  ...
                  #f)
              (cond
                [(not (= (char-rank cn) 2)) #f] ; Rejects "-" among others
                [(char=? c0 #\.) #f]
                [(char=? c0 #\0)
                 (cond
                   [(= len 1) #t] ; Accepts "0" but not "-0"
      ...
      (cond
        [(narrow-number? string) (string->number string)]
...
(define word
  (let ([string->word string->word]
        ...
        [whoops whoops])
    (lambda x
      (string->word (apply string-append
                           (map (lambda (arg)
                                  (if (word? arg)
                                      (word->string arg)
...
(define last
  (let ([pair? pair?]
        ...
        [word? word?])
    (define (word-last wd)
      (let ([s (word->string wd)]) (char->word (string-ref s (- (string-length s) 1)))))
...
(define bf
  (let ([pair? pair?]
        ...
        [whoops whoops]
        [word? word?])
    (define string-bf (lambda (s) (substring s 1 (string-length s))))
    (define (word-bf wd)
...
(define item
  (let ([> >]
    ...
    (lambda (n stuff)
      (cond
        [(not (integer? n)) (whoops "Invalid first argument to ITEM (must be an integer): " n)]
        [(< n 1) (whoops "Invalid first argument to ITEM (must be positive): " n)]
        [(> n (count stuff)) (whoops "No such item: " n stuff)]
    ...
    (define (vector-equal? v1 v2)
      (let ([len1 (vector-length v1)]
        ...
        (define (helper i)
          (if (= i len1)
              #t
              (and (equal? (vector-ref v1 i) (vector-ref v2 i)) (helper (+ i 1)))))
        (if (= len1 len2)
            (helper 0)
            #f)))
    (lambda (x y)
      (cond
        ...
        [(pair? y) #f]
        [(symbol? x)
         (or (and (symbol? y) (eq? x y)) (and (string? y) (string=? (symbol->string x) y)))]
        [(symbol? y) (and (string? x) (string=? x (symbol->string y)))]
        [(number? x)
         (or (and (number? y) (= x y))
             (and (string? y)
                  (let ([possible-num (string->word y)])
                    (and (number? possible-num) (= x possible-num)))))]
        [(number? y)
         (and (string? x)
              (let ([possible-num (string->word x)])
                (and (number? possible-num) (= possible-num y))))]
...
(define member?
  (let ([> >]
    ...
    (define (symbol-in-list? symbol string lst)
      (cond
        [(null? lst) #f]
        [(and (symbol? (car lst)) (eq? symbol (car lst)))]
        [(string? (car lst))
         (cond
           [(not string) (symbol-in-list? symbol (symbol->string symbol) lst)]
    ...
    (lambda (x stuff)
      (cond
        ...
        [(not (list? stuff)) (whoops "Invalid second argument to MEMBER?: " stuff)]
        [(symbol? x) (symbol-in-list? x #f stuff)]
        [(or (number? x) (string? x)) (word-in-list? x stuff)]
...
(define before?
  (let ([not not]
    ...
    (lambda (wd1 wd2)
      (cond
        [(not (word? wd1)) (whoops "Invalid first argument to BEFORE? (not a word): " wd1)]
        [(not (word? wd2)) (whoops "Invalid second argument to BEFORE? (not a word): " wd2)]
  ...
  (let ([null? null?]
    ...
    (lambda (pred l)
      ...
      (define (real-filter l)
        (cond
          [(null? l) '()]
          [(pred (car l)) (cons (car l) (real-filter (cdr l)))]
          [else (real-filter (cdr l))]))
      (cond
        [(not (procedure? pred)) (whoops "Invalid first argument to FILTER (not a procedure): " pred)]
        [(not (list? l)) (whoops "Invalid second argument to FILTER (not a list): " l)]
        [else (real-filter l)]))))
    ...
    (lambda (pred w-or-s)
      ...
      (define (keep-word wd)
        (let* ([string (word->string wd)]
               [len (string-length string)])
          (string->word (keep-string string 0 (make-string len) 0 len))))
      (cond
        [(not (procedure? pred)) (whoops "Invalid first argument to KEEP (not a procedure): " pred)]
        [(pair? w-or-s) (filter pred w-or-s)]
        [(word? w-or-s) (keep-word w-or-s)]
        [(null? w-or-s) '()]
        [else (whoops "Bad second argument to KEEP (not a word or sentence): " w-or-s)]))))

(define appearances
  (let ([count count]
        [keep keep]
        [equal? equal?])
    (lambda (item aggregate) (count (keep (lambda (element) (equal? item element)) aggregate)))))
  ...
  (let ([= =]
    ...
    (lambda (fn stuff)
      (define (string-every string i length)
        (if (= i length)
            '()
            (se (fn (char->word (string-ref string i))) (string-every string (+ i 1) length))))
      ...
      (cond
        [(not (procedure? fn)) (whoops "Invalid first argument to EVERY (not a procedure):" fn)]
        [(word? stuff)
         (let ([string (word->string stuff)]) (string-every string 0 (string-length string)))]
    ...
    (lambda (combiner stuff)
      ...
      (cond
        [(not (procedure? combiner))
         (whoops "Invalid first argument to ACCUMULATE (not a procedure):" combiner)]
...
(define reduce
  (let ([null? null?]
    ...
    (lambda (combiner stuff)
      ...
      (cond
        [(not (procedure? combiner))
         (whoops "Invalid first argument to REDUCE (not a procedure):" combiner)]
      ...
      (cond
        ...
        [(= (length args) 2)
         (if (not (output-port? (car (cdr args))))
             (whoops "Invalid second argument to SHOW (not an output port): " (car (cdr args))))
         (apply display args)
    ...
    (lambda (line . args)
      (if (>= (length args) 2)
          ...
          (let ([port (if (null? args)
                          ...
                          (car args))])
            (cond
              [(not (list? line)) (whoops "Invalid argument to SHOW-LINE (not a list):" line)]
              [(null? line) #f]
              [else
  ...
  (let ([read-char read-char]
    ...
    (define (read-string-helper chars all-length chunk-length port)
      (let ([char (read-char port)]
            [string (car chars)])
        (cond
          [(or (eof-object? char) (eqv? char #\newline))
           (apply string-append (reverse (cons (substring (car chars) 0 chunk-length) (cdr chars))))]
          [(>= chunk-length 80)
           (let ([newstring (make-string 80)])
             (string-set! newstring 0 char)
             (read-string-helper (cons newstring chars) (+ all-length 1) 1 port))]
          [else
           (string-set! string chunk-length char)
           (read-string-helper chars (+ all-length 1) (+ chunk-length 1) port)])))
    (lambda args
      (if (>= (length args) 2)
        ...
        (define (helper i start len)
          (cond
            ...
            [(char-whitespace? (string-ref string i))
             (if (= i start)
                 (helper (+ i 1) (+ i 1) len)
                 (cons (string->word (substring string start i)) (helper (+ i 1) (+ i 1) len)))]
    ...
    (lambda (obj width . rest)
      (define (align-number obj width rest)
        (let* ([sign (< obj 0)]
               ...
               [pos-str (if (>= (string-length cvt0) prec)
                            cvt
                            (string-append (make-string (- prec (string-length cvt0)) #\0) cvt))]
               [string (if sign
                           (string-append "-" pos-str)
                           pos-str)]
               [length (+ (string-length string) (if (= prec 0) 0 1))]
               [left (- length (+ 1 prec))]
               [result (if (= prec 0)
                           string
                           (string-append (substring string 0 left)
                                          "."
                                          (substring string left (- length 1))))])
          (cond
            [(= length width) result]
            [(< length width) (string-append (make-string (- width length) #\space) result)]
            [else
             (let ([new (substring result 0 width)])
      ...
      (define (align-word string)
        (let ([length (string-length string)])
          (cond
            [(= length width) string]
            [(< length width) (string-append string (make-string (- width length) #\space))]
            [else
             (let ([new (substring string 0 width)])
               (string-set! new (- width 1) #\+)
               new)])))
      (if (number? obj)
...
(define maybe-num
  (let ([string? string?]
    ...
    (lambda (arg)
      (if (string? arg)
          (let ([num (string->number arg)]) (if num num arg))
          arg))))
...
(define logoize-1 (let ([maybe-num maybe-num]) (lambda (fn) (lambda (x) (fn (maybe-num x))))))

(define logoize-2
  (let ([maybe-num maybe-num]) (lambda (fn) (lambda (x y) (fn (maybe-num x) (maybe-num y))))))

(define (strings-are-numbers yesno)
  (show "strings-are-numbers is disabled in Racket compatibility mode."))
